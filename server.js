...
async function runStartupCleanup() {
  try {
    ...
    try {
      // auto-approve legacy users with no status field
      await client.query(`
...
app.post('/api/login', async (req, res) => {
  ...
  // users with no status = old accounts → auto-approve
  if (!user.status) {
    user.status = 'approved';
    writeCollection('users', users).catch(() => {}); // async, don't block login
  }
  // migrate plain-text → hashed on successful login (non-blocking)
  if (bcrypt && user.password && !user.password.startsWith('$2')) {
    hashPassword(password).then(hashed => {
      user.password = hashed;
      writeCollection('users', users).catch(() => {});
    }).catch(() => {});
  }
...
app.post('/api/quizzes/cleanup', async (req, res) => {
  try {
    const client = await _pool.connect();
    try {
      // keep only one per title, delete duplicates
      const r = await client.query(`
        DELETE FROM "quizzes" WHERE id NOT IN (
          SELECT DISTINCT ON ((data->>'title')) id
          FROM "quizzes" ORDER BY (data->>'title'), id DESC
